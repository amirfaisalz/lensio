...
class ApiClient {
  ...
  public async revokeAPIKey(id: string): Promise<{ message: string; id: string }> {
  ...
  public async updateAccountPlan(planCode: string): Promise<{ message: string; plan_code: string }> {
    const res = await fetch(`${API_BASE}/api/v1/account/plan`, {
      method: 'PUT',
      headers: this.getHeaders(),
