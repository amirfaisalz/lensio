...
export const APIKeysPage: React.FC = () => {
  ...
  const [newKeyScopes, setNewKeyScopes] = useState<string[]>(['ocr:read', 'ocr:write', 'usage:read']);
  ...
  return (
    <div className="space-y-6 animate-in fade-in duration-200">
      ...
      <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4">
        <div>
          <h2 className="text-xl font-bold text-slate-900 tracking-tight">API Key Management</h2>
          <p className="text-xs text-slate-500">
            Securely generate, scope, and manage authentication credentials for your client applications.
          </p>
        </div>
      ...
      {error && (
        <div className="p-4 bg-rose-50 border border-rose-200 rounded-xl text-xs text-rose-700 flex items-center justify-between">
          <span>{error}</span>
          <button type="button" onClick={loadKeys} className="font-semibold underline">Retry</button>
        </div>
      )}
              ...
              {isLoading ? (
                ...
                <tr>
                  <td colSpan={7} className="px-6 py-12 text-center text-slate-500">
                    ...
                    <p className="font-semibold text-slate-700">No API Keys Generated</p>
                    <p className="text-xs text-slate-400 mt-1">Create an API key to start authenticating requests to NusaID.</p>
                  </td>
                </tr>
              ) : (
                keys.map((k) => {
                  ...
                  return (
                    <tr key={k.id} className="hover:bg-slate-50/80 transition-colors">
                      <td className="px-6 py-4">
                        ...
                        <span className="text-[10px] text-slate-400 font-mono">ID: {k.id.slice(0, 8)}...</span>
                      </td>
                      ...
                      <td className="px-6 py-4 text-slate-500 tabular-nums">
                        {k.last_used_at
                          ? new Date(k.last_used_at).toLocaleDateString(undefined, {
                              month: 'short',
                              day: 'numeric',
                              hour: '2-digit',
                              minute: '2-digit',
      ...
      >
        <form onSubmit={handleCreateKey} className="space-y-4">
          <div>
            <label htmlFor="key-name" className="block text-xs font-medium text-slate-700 mb-1">
          ...
          <div>
            ...
            <div className="grid grid-cols-2 gap-3">
              <label className={`flex items-center justify-between p-3 rounded-lg border cursor-pointer text-xs ${newKeyEnv === 'live' ? 'border-[#1877F2] bg-[#E7F3FF]/40 text-[#1877F2] font-semibold' : 'border-slate-200 text-slate-700'}`}>
              ...
              <label className={`flex items-center justify-between p-3 rounded-lg border cursor-pointer text-xs ${newKeyEnv === 'test' ? 'border-[#1877F2] bg-[#E7F3FF]/40 text-[#1877F2] font-semibold' : 'border-slate-200 text-slate-700'}`}>
          ...
          <div>
            ...
            <div className="space-y-2">
              {[
                { id: 'ocr:read', label: 'ocr:read', desc: 'Query previous OCR results and metadata' },
                { id: 'ocr:write', label: 'ocr:write', desc: 'Submit KTP documents for OCR extraction' },
                { id: 'usage:read', label: 'usage:read', desc: 'Read analytics, metrics, and logs' },
              ].map((scope) => (
                <label
                  key={scope.id}
        ...
        >
          <div className="space-y-4">
            <div className="p-3 bg-amber-50 border border-amber-200 rounded-lg flex items-start gap-2 text-xs text-amber-800">
              ...
              <div>
                <strong>Important:</strong> Please copy this API key now. For your security, this plaintext token will <strong>never be shown again</strong>.
              </div>
            </div>
        ...
        >
          <div className="space-y-3">
            <div className="flex items-center gap-2 text-rose-700">
              ...
              <h4 className="font-semibold text-sm">Are you sure you want to revoke this key?</h4>
            </div>
            <p className="text-xs text-slate-600 leading-relaxed">
              Revoking key <strong className="text-slate-900">{keyToRevoke.name}</strong> (<code className="font-mono text-[11px]">{keyToRevoke.prefix}••••</code>) will immediately reject all future requests using it with HTTP 401 Unauthorized. This action cannot be undone.
            </p>
          </div>
        </Modal>
      )}
