...
import { BarChart2, Layers, Clock, RefreshCw } from 'lucide-react';
      ...
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-xs text-slate-500">
            Historical request volume, endpoint partitioning, and latency metrics for current billing period.
          </p>
        </div>
        <button
          type="button"
      ...
      <div className="bg-white p-6 rounded-xl border border-slate-200 shadow-xs">
        <div className="flex items-center justify-between mb-6">
          <div>
            <div className="flex items-center gap-2">
              ...
              <h3 className="text-sm font-semibold text-slate-900">Daily Request Volume</h3>
            </div>
            <p className="text-xs text-slate-500">Distribution of successful vs error requests across the billing cycle.</p>
          </div>
        ...
        {isLoading ? (
          ...
          <div className="h-48 flex flex-col items-center justify-center text-slate-400 text-xs text-center">
            ...
            <p className="text-[11px] text-slate-500 mt-1">Make requests to `/api/v1/ocr/ktp` to view daily activity.</p>
          </div>
        ) : (
          <div className="space-y-3">
            <div className="h-44 flex items-end gap-2 pt-4 border-b border-slate-100 pb-2">
              {daily.map((d) => {
                const totalH = Math.max(8, Math.round((d.total_requests / maxDailyCount) * 100));
                const errorPct = d.total_requests > 0 ? (d.error_count / d.total_requests) : 0;
                ...
                return (
                  <div key={d.date} className="flex-1 flex flex-col items-center group relative h-full justify-end">
                    ...
                    <div className="opacity-0 group-hover:opacity-100 transition-opacity absolute -top-12 bg-slate-900 text-white text-[10px] rounded px-2 py-1 pointer-events-none whitespace-nowrap shadow-lg z-10 font-mono">
                      <div>{d.date}</div>
                      <div>Total: {d.total_requests} ({d.success_count} ok, {d.error_count} err)</div>
                    </div>
      ...
      <div className="bg-white rounded-xl border border-slate-200 shadow-xs overflow-hidden">
        <div className="px-6 py-4 border-b border-slate-100 flex items-center justify-between">
          <div className="flex items-center gap-2">
            ...
            <h3 className="text-sm font-semibold text-slate-900">Endpoint Breakdown</h3>
          </div>
          <span className="text-xs text-slate-500">{endpoints.length} Active Route(s)</span>
        </div>
          ...
          <table className="w-full text-left text-xs text-slate-600">
            ...
            <tbody className="divide-y divide-slate-100 font-medium">
              {isLoading ? (
                ...
                endpoints.map((ep) => {
                  const totalAll = endpoints.reduce((acc, e) => acc + e.total_requests, 0) || 1;
                  const sharePct = ((ep.total_requests / totalAll) * 100).toFixed(1);

                  return (
                    <tr key={ep.endpoint} className="hover:bg-slate-50/80 transition-colors">
                      ...
                      <td className="px-6 py-4">
                        <div className="flex items-center gap-3">
                          ...
                          <span className="text-slate-500 font-mono text-[11px]">{sharePct}%</span>
                        </div>
                      </td>
