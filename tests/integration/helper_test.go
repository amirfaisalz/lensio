...
func obtainTestAuthClient(client *http.Client, apiURL string) (string, string, error) {
	...
	if err == nil && loginResp.StatusCode == http.StatusOK {
		...
		for _, c := range loginResp.Cookies() {
			if c.Name == "lensio_session" && c.Value != "" {
		...
		if loginResult.Organization != nil && loginResult.Organization.ID != "" {
			...
			// 4. Create Organization (authenticated via session cookie / token)
			...
			if sessionToken != "" {
				...
				req.AddCookie(&http.Cookie{Name: "lensio_session", Value: sessionToken})
			}
