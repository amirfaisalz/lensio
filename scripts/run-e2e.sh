#!/usr/bin/env bash
# ==============================================================================
# NusaID Playwright End-to-End Test Suite Runner (Phase 9 - PRD Section 28)
# ==============================================================================
# Orchestrates local/CI environment:
#   1. Starts NusaID Go API on port 8080
...
DB_URL="${DATABASE_URL:-postgres://nusaid:nusaid_dev_password@localhost:5432/nusaid?sslmode=disable}"

echo "======================================================================"
echo " 🎭 NusaID Playwright E2E Suite Runner (PRD Section 28)"
echo "======================================================================"

# 1. Start Go API Server
echo "🚀 Starting NusaID API on ${API_URL}..."
PORT="${PORT}" DATABASE_URL="${DB_URL}" ENV="test" LOG_LEVEL="warn" \
    go run ./apps/api/cmd/server > /tmp/nusaid_api_e2e.log 2>&1 &
API_PID=$!
...
for i in $(seq 1 30); do
    if curl -s "${API_URL}/ready" | grep -q '"database":"connected"'; then
        ...
        if command -v docker >/dev/null 2>&1 && docker ps | grep -q nusaid-postgres; then
            docker exec nusaid-postgres psql -U nusaid -d nusaid -c \
...
if [ "${READY}" = false ]; then
    ...
    cat /tmp/nusaid_api_e2e.log
    exit 1
fi

# 2. Start Dashboard Vite Dev Server
echo "🚀 Starting React Dashboard on ${DASHBOARD_URL}..."
npm --prefix apps/dashboard run dev -- --port 3000 > /tmp/nusaid_dash_e2e.log 2>&1 &
DASH_PID=$!
